...
class MessagesController extends Controller
{
    ...
    public function investors($id = null)
    {
        // get current route
        $route = (in_array(\Request::route()->getName(), ['user', config('chatify.path')]))
            ? 'user'
    ...
    public function getFavorites(Request $request)
    {
        ...
        $user = \Auth::user();
        if ($user->role_id === 3) {
            $favorites = User::where('role_id', 2);
            foreach ($favorites->get() as $favorite) {
                // get user data
                $user = User::where('id', $favorite->id)->first();
    ...
    public function search(Request $request)
    {
        ...
        $user = \Auth::user();
        if ($user->role_id === 3) {
            $records = User::where('name', 'LIKE', "%{$input}%")->where('role_id', 2);
        }else{
